...
var TICKER='$BABYSNOWBALL';
var CA='0xee9c021d159fde02a0262c609449d871aea65555';
var TWITTER='https://x.com/babysnowballbsb?s=11';
var TG='https://t.me/babysnow_ball';
var WEBSITE='https://babysnowball.tokly.io/';
var IS_CTO=true;
...
loadState();
var _IMG1=path.join(__dirname,'babysnowball.jpg');
...
function chatSys(){return 'You are a friendly, casual member of the $BABYSNOWBALL Telegram community. When asked to post something, output ONLY the exact message to send to the group - no quotes, no preamble, no explanation, no describing the community. Write like a real person texting: short, lowercase-ish, natural. Never use hype, moon, or price-prediction language. Never mention contract/renounced/LP. 1-2 lines max.';}
...
async function doShoutout(){
  ...
  try{
    ...
    var ppt='Write 1-2 warm genuine lines appreciating these $BABYSNOWBALL admins for keeping the community alive: '+names.join(', ')+'. Be specific, sound human, tag them by name.';
...
bot.command('ca',async function(ctx){if(!caUnlocked)return ctx.reply(NOT_LIVE[Math.floor(Math.random()*NOT_LIVE.length)]);await sendWithTracker(caMsg,ctx.chat.id,'$BABYSNOWBALL Contract Address',{});return ctx.reply('<code>'+CA+'</code>',{parse_mode:'HTML'});});
bot.command('x',async function(ctx){return sendWithTracker(xMsg,ctx.chat.id,'Follow $BABYSNOWBALL on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});});
bot.command('twitter',async function(ctx){return sendWithTracker(xMsg,ctx.chat.id,'Follow $BABYSNOWBALL on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});});
bot.command('socials',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xee9c021d159fde02a0262c609449d871aea65555\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xee9c021d159fde02a0262c609449d871aea65555\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('links',function(ctx){return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xee9c021d159fde02a0262c609449d871aea65555\'>Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xee9c021d159fde02a0262c609449d871aea65555\'>PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':'')+(WEBSITE?' | <a href=\''+WEBSITE+'\'>Website</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('info',function(ctx){return ctx.reply('<b>$BABYSNOWBALL</b> \u2014 BNB Smart Chain (BSC)\n\nSupply: N/A\nTax: 3% buy / 3% sell\nContract: RENOUNCED\nLP: LOCKED'+(TWITTER?'\nTwitter: '+TWITTER:''),{parse_mode:'HTML',disable_web_page_preview:true});});
bot.command('shill',async function(ctx){
  var shillMsgs=[
    'Have you heard about $BABYSNOWBALL?\n\n$BABYSNOWBALL \u2014 community-owned on BSC.\nRenounced. LP LOCKED. No dev games.\nThis is the quiet move. Load up.',
    'Looking for a BSC token built by real people?\n\n$BABYSNOWBALL \u2014 fully community-owned.\nRenounced contract. LP LOCKED. Real narrative.\nGet in early \u261d',
    'The move others will regret missing.\n\n$BABYSNOWBALL on BSC \u2014 community takeover.\nRenounced. LP LOCKED. No rug possible.\nLoad up before it runs.',
    'What if the next gem was right here?\n\n$BABYSNOWBALL \u2014 zero dev, 100% community.\nRenounced. LP LOCKED. Low cap. Real conviction.',
    'Don\u2019t sleep on $BABYSNOWBALL.\nCommunity took over. Dev is gone. LP LOCKED.\nThis is what conviction looks like. Load up.',
  ];
...
bot.on('message',async function(ctx){
  ...
  if(admin&&!isPrivate){
    ...
    if(caW.some(function(w){return lower===w||lower.includes(w);})){
      ...
      await sendWithTracker(caMsg,ctx.chat.id,'$BABYSNOWBALL Contract Address',{});return ctx.reply('<code>'+CA+'</code>',{parse_mode:'HTML'});
    }
    if(lower==='x'||lower==='twitter')return sendWithTracker(xMsg,ctx.chat.id,'Follow $BABYSNOWBALL on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});
    if(lower==='socials'||lower==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xee9c021d159fde02a0262c609449d871aea65555\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xee9c021d159fde02a0262c609449d871aea65555\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
    return;
  }
  if(!text)return;
  ...
  if(lower2==='x'||lower2==='twitter'||lower2.includes('follow on'))return sendWithTracker(xMsg,ctx.chat.id,'Follow $BABYSNOWBALL on X',{reply_markup:{inline_keyboard:[[{text:'Follow on X',url:TWITTER}]]}});
  if(lower2==='socials'||lower2==='links')return ctx.reply('<a href=\'https://dexscreener.com/bsc/0xee9c021d159fde02a0262c609449d871aea65555\'> Chart</a> | <a href=\'https://pancakeswap.finance/swap?outputCurrency=0xee9c021d159fde02a0262c609449d871aea65555\'> PancakeSwap</a>'+(TWITTER?' | <a href=\''+TWITTER+'\'>Twitter</a>':''),{parse_mode:'HTML',disable_web_page_preview:true});
...
app.listen(PORT,async function(){console.log('$BABYSNOWBALL bot port '+PORT);try{await new Promise(function(r){setTimeout(r,2000);});}catch(_){}try{await regWH();}catch(e){console.log(e.message);}if(parseInt(SIL_DELAY||'0')>0)try{resetSil();}catch(_){}try{schedShout();}catch(_){}setInterval(function(){if(WEBHOOK_URL)try{fetch(WEBHOOK_URL+'/health').catch(function(){});}catch(_){}},4*60*1000);console.log('$BABYSNOWBALL bot live');});